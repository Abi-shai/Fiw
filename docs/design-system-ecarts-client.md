...
| 2 | ~~État de résultat plein écran~~ | **5** | `transport/cloture` · `livraison/cloture` · `affilie/retrait-confirmation` · `affilie/retrait-echec` · `affilie/celebration` | ✅ **`ResultState` construit** (`768:1623`) — `Ton=succès\|erreur\|accent`, props `Titre`/`Corps`, actions déléguées au `ScreenFooter`. **`marque` manque** : il attend un jeton d'opacité sur fond primaire (§4 n° 1). Reste à écrire côté code. |
| 3 | ~~Toast~~ | **2** | `affilie/outils.tsx:121` · `affilie/qr.tsx:86` | ✅ **`Toast` construit** (`764:259`) — `Libellé`, `Icône`, `IconName`. Reste à écrire côté code. |
| 4 | ~~Case à cocher~~ | **2, déjà divergentes** : 22 px / rayon 6 / `check` 14 vs 24 px / `radius/sm` / `tick` 16 | `affilie/presentation.tsx:130` · `affilie/conditions.tsx:78` | ✅ **`Checkbox` construit** (`757:32`) — `Selected` × `État`, 24 px / `radius/sm` / liseré `thick`, aligné sur `Radio`. Reste à écrire côté code. |
| 5 | ~~Saisie de code (OTP)~~ | **2, divergentes** | `otp.tsx:100` · `compte/numero.tsx:146` · `livraison/suivi.tsx` (restitution) | ✅ **`CodeField` construit puis FUSIONNÉ avec `CodePill`** (`779:320`) — un seul composant, axe `État = vide · 1 · 2 · 3 · complet · erreur · lecture`. Benchmark Mobbin : seule la case active est marquée. `CodePill` supprimé (4 instances basculées) et la primitive orpheline `CodeCell` retirée. Retravaillé au relevé bancaire (Chime, Wise, State Farm) : cases portrait 60×68, curseur, liseré seul porteur de l'accent. Reste à écrire côté code. |
| 6 | ~~Interrupteur~~ | **5 rangées / 2 écrans** | `compte/securite.tsx:43` · `compte/preferences.tsx:24` | ✅ **`Toggle` construit** (`759:34`) — `On` × `État`, géométrie iOS 51×31 du `Switch` natif. Reste à écrire côté code. |
| 7 | ~~Contrôle segmenté~~ | **2, divergents** | `affilie/reseau.tsx:85` · `transport/configure.tsx` (`segmentItem`) | ✅ **`SegmentedControl` construit** (`760:57`) — `Segments=2\|3` × `Actif=0\|1\|2`, 5 variantes. Traitement retenu : piste `track` + pastille `surface` (celui de Transport). Reste à écrire côté code. |
...
| 10 | ~~Barre d'actions de pied d'écran~~ | **12 déclarations**, 4 géométries | 12 écrans | ✅ **`ScreenFooter` construit** (`766:1605`) — `Liseré:BOOLEAN`, slot `Actions`, padding 12/16/32. Reste à écrire côté code. |
| 11 | ~~Note d'aide~~ ✅ **`Hint` construit** (`765:256`) — `Texte`, `Icône`, `IconName`, `caption` tertiaire, gouttière 6. Reste à écrire côté code. | **8** : avec icône `outils.tsx:118` · `compte/numero.tsx:142` · `compte/profil.tsx:145` · `compte/lieu.tsx:423` ; sans icône `lieux.tsx:76` · `qr.tsx:78` · `profil.tsx:118` · `ChipGroup.tsx:96` | idem | `Hint` — `Icône=false\|true`. À distinguer explicitement de `Callout` (pastille jaune, une affordance qu'on énonce) et d'`InfoBanner` (bandeau de feuille) : la note d'aide ne porte ni fond ni pastille |
...
| ~~Indicateur de chargement~~ | `affilie/retrait-traitement.tsx:18` (+ `Button` loading) | ✅ **`Spinner` construit** (`761:30`) — `Taille=sm 20\|lg 36`, arc de 306°. Reste à écrire côté code. |
